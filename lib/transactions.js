...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  if(transaction.height >= 0) {
    confirmations = this.node.services.ImageCoind.height - transaction.height + 1;
  }
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  if (output.imgbase64) {
    transformed.imgbase64 = output.imgbase64;
  }
  ...
  if (output.address) {
    ...
    var address = imagecoincore.Address(output.address); //TODO return type from imagecoincore-node
