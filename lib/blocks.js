...
var imagecoincore = require('@mceme/imagecoin-lib');
var _ = imagecoincore.deps._;
var pools = require('../pools.json');
var BN = imagecoincore.crypto.BN;
...
BlockController.DEFAULT_BLOCKSUMMARY_CACHE_SIZE = 80000000;
BlockController.DEFAULT_BLOCK_CACHE_SIZE = 10000;
...
BlockController.prototype.checkBlockHash = function(req, res, next) {
  ...
  if (hash.length < 64 || !isHexadecimal(hash)) {
    this.node.services.ImageCoind.getBlockHeader(parseInt(hash), function(err, info) {
      if (err) {
...
BlockController.prototype.blockHeader = function(req, res, next) {
	...
	self.node.services.ImageCoind.getBlockHeader(hash, function(err, info) {
		if (err) {
			return self.common.handleErrors(err, res);
		}
...
BlockController.prototype.blockHeaders = function(req, res, next) {
	...
	if(blockIdentifier.length===64){
		...
		var result = self.node.services.ImageCoind.getBlockHeaders(hash, cb, nbOfBlockToFetch);
	}else{
		var height = blockIdentifier;
		var result = self.node.services.ImageCoind.getBlockHeaders(height, cb, nbOfBlockToFetch);
	}
...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    blockCached.confirmations = self.node.services.ImageCoind.height - blockCached.height + 1;
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.ImageCoind.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.ImageCoind.getRawBlock(hash, function(err, blockBuffer) {
      if (err) {
        return next(err);
      }

      var br = new imagecoincore.encoding.BufferReader(blockBuffer);
      ...
      var header = imagecoincore.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [imagecoincore.Transaction().fromBufferReader(br)];

      self.node.services.ImageCoind.getBlockHeader(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = self.node.services.ImageCoind.height - height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  self.node.services.ImageCoind.getBlockHashesByTimestamp(lte, gte, function(err, hashes) {
    if(err) {
      return self.common.handleErrors(err, res);
    }
...
BlockController.prototype.getPreviousBlock = function(prevHash, cb) {
  ...
  if(prevHash===null) cb(null,null);
      ...
      self.node.getBlock(prevHash, function (err, block) {
          ...
          self.node.services.ImageCoind.getBlockHeader(prevHash, function (err, info) {
              if (err) {
